<div class="container font-montserrat" id="hero">
  <div class="h-screen flex flex-col justify-center p-2">
    <div class="mx-auto">
      <img
        src="/dp.png"
        class="w-48 mx-auto shadow-2xl rounded-full m-3"
        alt=""
      />
      <p class="text-xl">Hi! I'm</p>
      <p
        class="md:text-6xl text-4xl font-bold py-2 font-montserrat bg-clip-text text-transparent bg-gradient-to-r from-red-500 to-blue-600"
      >
        Maniteja pratha
      </p>
      <ul class="md:text-xl">
        <li class="py-2">🚀 Google DSC Lead</li>
        <li class="py-2">⭐ Microsoft learn student ambassador</li>
        <li class="py-2">👩‍💻 Web and mobile app developer</li>
      </ul>
      <ul class="flex space-x-2 py-3">
        <a
          href="https://twitter.com/PrataManitej"
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
            class="w-8"
            src="https://camo.githubusercontent.com/35b0b8bfbd8840f35607fb56ad0a139047fd5d6e09ceb060c5c6f0a5abd1044c/68747470733a2f2f6564656e742e6769746875622e696f2f537570657254696e7949636f6e732f696d616765732f7376672f747769747465722e737667"
            alt=""
          />
        </a>
        <a
          href="https://twitter.com/PrataManitej"
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
            class="w-8"
            src="https://camo.githubusercontent.com/c9dacf0f25a1489fdbc6c0d2b41cda58b77fa210a13a886d6f99e027adfbd358/68747470733a2f2f6564656e742e6769746875622e696f2f537570657254696e7949636f6e732f696d616765732f7376672f696e7374616772616d2e737667"
            alt=""
          />
        </a>
        <a
          href="https://github.com/Manitej66"
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
            class="w-8"
            src="https://camo.githubusercontent.com/b079fe922f00c4b86f1b724fbc2e8141c468794ce8adbc9b7456e5e1ad09c622/68747470733a2f2f6564656e742e6769746875622e696f2f537570657254696e7949636f6e732f696d616765732f7376672f6769746875622e737667"
            alt=""
          />
        </a>
        <a
          href="https://dev.to/manitej"
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
            class="w-8"
            src="https://camo.githubusercontent.com/6cc90061976bcd4d1a61a6c76b818538b5a65754f7b7b8068fe0fa49a09def8f/68747470733a2f2f6564656e742e6769746875622e696f2f537570657254696e7949636f6e732f696d616765732f7376672f6465765f746f2e737667"
            alt=""
          />
        </a>
        <a
          href="https://www.linkedin.com/in/maniteja-pratha-2a1469197/"
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
            class="w-8"
            src="https://camo.githubusercontent.com/c8a9c5b414cd812ad6a97a46c29af67239ddaeae08c41724ff7d945fb4c047e5/68747470733a2f2f6564656e742e6769746875622e696f2f537570657254696e7949636f6e732f696d616765732f7376672f6c696e6b6564696e2e737667"
            alt=""
          />
        </a>
      </ul>
    </div>
  </div>
</div>
